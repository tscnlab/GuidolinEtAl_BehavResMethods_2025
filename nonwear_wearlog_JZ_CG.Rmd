...
nonwear_duration <- ggplot(nonwear_df_daily, aes(x = day, y = nonwear_daily)) +
  ...
  ggtitle("Participants' (n=26) non-wear time during the experimental week\n according to self-reported wear log entries")
...
Let's save this 
```{r}
ggsave(filename = "nonwearduration6.png",
       plot = nonwear_duration,
       width = 7,
       height = 8,
       path= "D:/cyepi/code/pre-processing/nonwear_detection")
```
...
nonwear_df_daily_binned <- nonwear_df_daily %>%
  mutate(centre_point_binned = cut(centre_point_format, breaks = "30 min"))
...
ggplot(nonwear_df_daily_binned, aes(x = centre_point_binned, y = ..density..)) +
  ...
  scale_x_datetime(
    ...
    expand = c(0, 0),
    limits = c(as.POSIXct("2023-11-17 01:00"), as.POSIXct("2023-11-18 01:00"), tz = tz )
  ) +
   scale_y_continuous(
