...
#These were corrected directly on the csv file as they all stemmed from the participant accidenatlly logging the wrong data when reporting sleep and bed dates from the previous day. 
```
```{r}
...
int_duration_sleep_clean <- int_duration_sleep %>%
  ...
  filter(!Id == "213") #we eliminate participant 213 from this visualisation since on einterval of time for the sleep period is missing, which would cause the data to look diferent. 
