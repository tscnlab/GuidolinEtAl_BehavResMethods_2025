...
```{r}
dataset.LL.mlit <- data.LL.filtered %>%
...
y_limits <- as.POSIXct(c("1970-01-01 11:30", "1970-01-01 16:00"))
y_limits <- force_tz(y_limits, tzone = tz)
...
```{r}
y_limits2 <- as.POSIXct(c("1970-01-01 09:30", "1970-01-01 17:00"))
...
violin <- ggplot(merged_mlit_msf, aes(x = chronotype_cat, y = mlit.250)) +
  ...
  scale_fill_manual(values = c("#FFCC00", "#FF9900", "#FF6633")) +
   scale_y_datetime(limits = y_limits2, breaks = date_breaks("1 hour"), labels = date_format("%H:%M"), timezone = tz) +
  theme(legend.position = "none") +
#  xlab(bquote("Chronotype "(MSF[sc]))) + 
#  ylab(bquote("Mean light (mEDI) timing above 250 lx  "(MLiT^250))) +
...
```{r}
library(svglite)
ggsave("chronotype_multiplot_number3.png", plot =  chrono_multiplot, width = 8, height = 4)
```
