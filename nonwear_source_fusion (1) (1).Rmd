...
dis_daytype <- ggplot(data = weekly_per_nonwear, aes(x = per_nonwear, y= day_type, fill = day_type))  +
 # xlim(NA, 100) +
  ggdist::stat_slab(aes(thickness = after_stat(pdf*n)), scale = 0.5) +
...
tot_nonwear <- aggregate(off_time~Id, data = off_states, sum)

tot_nonwear$offtime_per <- (tot_nonwear$off_time / (24*7))*100

nonwear_dist_overview <- ggplot(data = tot_nonwear, aes(x = offtime_per))  +
  xlim(NA, 100) +
  ggdist::stat_slab(aes(thickness = after_stat(pdf*n)), fill = "#16439C", scale = 0.5) +
  ggdist::stat_dotsinterval(side = "bottom", scale = 0.3, slab_colour = "#16439C", slab_linewidth = NA, fill = "#16439C") +
  theme_classic() +
  labs(title = "Non-wear time distribution") +
  xlab("Non-wear time (%)") +
  ggpubr::rremove("ylab") +
