...
Since we have all of this information already loaded after we integrated the sources of non-wear in the df dataset.LL.clean (see nonwear_source_fusion_wlrg.Rmd file), we will use this instead of leading each each light logger dataset again from each participant.
...
ggsave(filename = filename,
       ...
       dpi = 600,
       bg = "white",
       path= "G:/cyepi/code/outputs/visualinspection2")
}
...
```
##Poster making
For the poster I am preparing for a conference, I would like to create a single plot for one representative participant. We choose participant 201 for this purpose. 
```{r}
...
p <- gg_day(data = subset_df, #base plot defaults to plotting mEDI on y axis
            ...
            x.axis.label = "Time", 
            y.axis.label = "Illuminance (mEDI, lx)",
            title = paste("Exemplary light exposure and (non-)wear time for n=1 participant")) +
...
ggsave(filename = "exemplaryrawreadings_2.svg",
       plot = p,
       width = 11,
       height = 14,
       dpi = 600,
       bg = "white",
       path= "G:/cyepi/code/outputs/visualinspection2/light/sltbr")
