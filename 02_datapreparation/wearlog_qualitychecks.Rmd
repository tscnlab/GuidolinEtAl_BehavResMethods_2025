...
# Note: run this script after having run the scripts in 01_import
These are: import_LL, import_wearlog and import_bag

# Joining of two datasets: wear log and bag data sets
Now we have a dataframe wearlog_bag_clean that contains timestamps for when the black bag was used or not used. We'd like to join this with the dataframe that contains the Wear log entries, i.e. wearlog_bag_clean. Afterwards, we will move on to perform quality checks on this combined df. 
...
# Performing quality checks for the Wear log and bag dataset 
...
corrected_bag = as.numeric(2) #value for bag also has to be corrected to 2, since no bag was used here

wrlg_entries <- joined_df %>%
  mutate(
    Datetime = if_else(Id == "202" & Datetime == "2023-08-14 16:42:00" & State == 1, corrected_202datetime_on, Datetime),
    bag =  if_else(Id == "202" & Datetime == corrected_202datetime_on, corrected_bag, bag))
...
#3Error: Consecutive timestamps at record_id: 201 datetime: 2023-08-17 06:51:00 wearlog_off (0) - participant wore the light logger for less than 1 minute, and then placed it in the black bag. To adjust for this, we will add 1 minute to the wear log entry: 2023-08-17 06:52:00. The value in the bag column should also be changed so that the 2023-08-17 06:51:00 entry has bag = 2 (since it is now an "on" entry). 
...
correctbag <- as.numeric(2)

wrlg_entries <- wrlg_entries %>%
  mutate(
    bag = ifelse(Id == "201" & Datetime == "2023-08-17 06:51:00" & State == 1, correctbag, bag)
  )

#4Error: Consecutive timestamps at record_id: 201 datetime: 2023-08-17 17:40:00 wearlog_on (1) - participant accidentally entered incorrect datetime. Correct value can be taken from start_date column: 17-08-2023 11:40:00. Because this is an on entry, we also must ensure that the bag value is 2. 
...
wrlg_entries <- wrlg_entries %>%
  mutate(
    bag = if_else(Id == "201" & Datetime == "2023-08-17 11:40:00" & State == 1, correctbag, bag)
  )
...
#8Error: Consecutive timestamps at record_id: 214 datetime: 2023-09-26 23:15:00 wearlog off (0) - entry for participant was accidentally entered twice. The second one is a wearlog_bed entry, which participant specified being the correct one in email exchange. This, the wearlog_off entry should be removed. The bag value of the wearlog_bed entry should then also be 2. 

correctbag <- as.numeric(2)

wrlg_entries <- wrlg_entries%>%
  filter(!(Id == "214" & Datetime == "2023-09-26 23:15:00" & State == 0)) %>%
  mutate(bag = if_else(Id == "214" & Datetime == "2023-09-26 23:15:00" & State == "2", correctbag, bag))
...
```
## While cleaning the entries above and performing a visual check, we notice another behaviour in PID 214
Namely, there is no sleep entry for the first night of the experiment. Going back to the participant sheet and the email exchanges, we know that they logged this as off entry vs. sleep entry. We change this manually. 

```{r}
correct_state214 <- "2"
correct_bag = as.numeric(2)

wrlg_entries <- wrlg_entries%>%
  mutate(State = if_else(Id == "214" & Datetime == "2023-09-25 22:45:00" & State == "0", correct_state214, State)) %>%
  mutate(bag = if_else(Id == "214" & Datetime == "2023-09-25 22:45:00" & State == "2", correct_bag, bag))
```
...
#1Solution: This is the same entry that caused issues in the Quality check #1. We had changed it above to a "wearlog_on" entry from the start_date column, which appears to be the first entry after wake up with timestamp 7:47. Since I had not noticed the consecutive datetime error above, I had asked the participant to make a retrospective entry,which they logged as 07:24. Now, we eliminate the retrospective entry since we have found the original "current" entry, on the basis that the restropsective entry would be less accurate. 

wrlg_entries <- wrlg_entries %>%
  filter(!(Id == "202" & Datetime == "2023-08-15 07:24:00" & State == 1))
...
#Since we have turned this into an on (1) entry, we should also change the bag value to 2. 
correctbag <- as.numeric(2)

wrlg_entries <- wrlg_entries %>%
  mutate(
    bag = if_else(Id == "201" & Datetime == "2023-08-18 19:54:00" & State == 1, correctbag, bag)
  )
...
wrlg_entries <- wrlg_entries %>%
  mutate(
    bag = if_else(Id == "205" & Datetime == "2023-09-02 16:47:00" & State == 0, correctbag, bag)
  )
...
#1) 2023-09-03 08:47:00 wearlog_on should be turned into 2023-09-03 08:40:00 wearlog_on, and bag should be turned to 2
#2) 2023-09-03 08:40:00 wearlog_on should be turned into 2023-09-03 08:47:00 wearlog_off, and bag should be turned to 0 (as reported by participants)
#3) 2023-09-03 09:40:00 wearlog_off  should be left as is, and bag should also be left at a value of 2

#1)
correct_state205_1 <- "1"
correct_bag205_1 <- as.numeric(2)
wrlg_entries <- wrlg_entries %>%
  mutate(
    State = if_else(Id == "205" & Datetime == "2023-09-03 08:40:00" & State == 0, correct_state205_1, State),
    bag = if_else(Id == "205" & Datetime == "2023-09-03 08:40:00", correct_bag205_1, bag))  

#2)
correct_state205_2 <- "0"
correct_bag205_2 <- as.numeric(0)

wrlg_entries <- wrlg_entries %>%
  mutate(
    State = if_else(Id == "205" & Datetime == "2023-09-03 08:47:00" & State == 1, correct_state205_2, State),
    bag = if_else(Id == "205" & Datetime == "2023-09-03 08:47:00", correct_bag205_2, bag)
  )
...
#14Solution: this is the last day of participation for this individual. We will only keep the first entry anyways, so we can eliminate this second entry which we would not use anyways (and is redundant) - without caring about what the participant actually wanted to log
...
#Participant 202. The first entry is a sleep entry, as the participant went to bed after midnight. However, the following entry is a State = 0, meaning an "off" entry, which should not be the case (State = 1 would be expected as "wake up" entry). In the participant documentation, I logged that they retrospectively logged the wearlog_on of the last day while they were at the lab already. This indicates that the participant accidentally logged wearlog_off instead of wearlog_on. This interpretation also fits with the data from the sleep diary, where they indicated that they got out of bed at 7:20:00 (3 minutes before they started wearing the glasses, according to the wearlog_on entry at 7:23:00). So we manually change this. We also change bag to 2. 
correct_state202_on <- "1"
correct_bag_202_on <- as.numeric(2)
wrlg_entries_clean <- wrlg_entries_clean %>%
  mutate(
    State = if_else(Id == "202" & Datetime == "2023-08-21 07:23:00" & State == 0, correct_state202_on, State),
    bag = if_else(Id == "202" & Datetime == "2023-08-21 07:23:00", correct_bag_202_on, bag))  
...
3) if State is either on or sleep, bag should be 2, whereas if state is off (0), bag can have values 0 or 1. 
```{r}
...
#Checking that values in State and bag correspond

perform_bag_check <- function(df) {
  for (i in 1:nrow(df)) {
    if (df$State[i] == 1 || df$State[i] == 2) {
      if (df$bag[i] != 2) {
        stop(paste("Quality check failed at index", i, ": State is", df$State[i], "but bag is not 2"))
      }
    } else if (df$State[i] == 0) {
...
tryCatch({
  perform_bag_check(wrlg_entries_clean)
