...
wearlog_intervals <- wearlog_intervals_raw %>%
  mutate(
...
wearlog_intervals <- bind_rows(wearlog_intervals, newrow_202) %>%
  arrange(Id, Datetime)
...
1) If two entries for the same State: one retrospective and one current, then we only keep the current [on basis that this is more accurate than retrospective]
...
3) If missing data for wearlog_on in the morning: take timestamp from outof_bed from sleep diary
...
5) If retrospective entries conflict with previously reported current entries (see #7error), we adjust this manually according to visual inspection of raw light data 
6) Retrospective entries which are not explainable by any visual inspection of the data: elimnate (see #11error)

###The wear log data is now clean from erraneous entries. However, it still contains unorganised entries for the last day of the experiment (Monday). On this day, the participants came back to the lab and returned the light glasses. We want to keep only the first "wearlog_on" entry of this day for each participant (State == 1). For participants where this is not possible because of missing data, we use the outof_bed entry from the wear log. 
```{r}
#Ensure the columns are formatted as the original wearlog_intervals_raw dataframe
wearlog_intervals_clean <- wearlog_intervals %>%
  mutate(State = as.character(State),
         Datetime = as.POSIXct(Datetime, tz = tz),
         Id = as.factor(Id))

#We calculate the experimental day for each wear log entry the participant had
wearlog_intervals_day <- wearlog_intervals_clean %>%
  group_by(Id) %>%
...
quality_check <- function(wearlog_intervals_day) {
  wearlog_intervals_day %>%
    group_by(Id) %>%
    arrange(Datetime) %>%
    summarise(
      result = case_when(
        n() == 1 & State[1] == "1" ~ "The first entry is on",
...
quality_check(wearlog_intervals_day)
...
```{r}
#From the code above, we want to fix the participants where we get the result "Check this participant" and "The first entry is on, but there are multiple entries". 

#Participant 202. The first entry is a sleep entry, as the participant went to bed after midnight. However, the following entry is a State = 0, meaning an "off" entry, which should not be the case. In the participant documentation, I logged that they retrospectively logged the wearlog_on of the last day while they were at the lab already. This indicates that the participant accidentally logged wearlog_off instead of wearlog_on. This deduction also fits with the data from the sleep diary, where they indicated that they got out of bed at 7:20:00 (3 minutes before they started waering the glasses, according to the wearlog_on entry at 7:23:00). So we manually change this. 
correct_state202_on <- "1"
wearlog_intervals_clean <- wearlog_intervals_clean %>%
  mutate(
...
#Participant 205: The participant has a second entry which is an "off" entry, as well as a third on entry. We do not care about these two last entries - so we flag them (see rows_todelete below) and will delete them in the original dataset

#Participant 206: The participant as 3 entries on the last day: one on entry after midnight when they started wearing the light glasses again, one sleep entry for when they removed them and one final on entry for when they started wearing them again in the morning. We need to keep all three of them. 
...
#However, this poses the problem that now we have no entry for participant 219 on the last day of the experiment. This is missing data, and we will need to get this timestamp from the sleep diary column out_ofbed. We flag this in a new dataframe newrows_on
...
#Participant 226. The first entry is a sleep entry (went to bed after midnight) and the second one, and last one, is an on entry. This makes sense and we want to keep both of them. There is also a third "off" entry which we would like to delete, and do so in the rows_todelete df
...
wearlog_intervals_clean <- anti_join(wearlog_intervals_clean, rows_todelete) 
#From 790 obs, we now have 781 obs, which shows that we have successfully delete the 9 entries we wanted. 
...
wearlog_intervals_clean <- bind_rows(wearlog_intervals_clean, newrows_on) %>%
  arrange(Id, Datetime)
#which gives us a final n of obs 786, meaning that we successfully added the 5 entries from newrows_on (781 + 5 = 786)

```

###Quality checks after the data cleaning
After having cleaned the data, we want to make sure that a couple of things are as they should:
1) The first timestamp for all participant should be a State == 1
2) The last timestamp should also be a State == 1
```{r}
wearlog_intervals_clean %>%
  group_by(Id) %>%
  summarise(
...
wrlg_int_states <- wearlog_intervals_clean %>%
   mutate(
      State = case_when(
...
#To check that the State coding into on, off and sleep worked, we want to see where NA values where assigned. We expect NA values for participant's first entry, as the previous entry is absent

first_tmp_int <- wrlg_int_states %>%
  group_by(Id) %>%
...
wrlg_int_states <- wrlg_int_states %>%
  group_by(Id) %>%
