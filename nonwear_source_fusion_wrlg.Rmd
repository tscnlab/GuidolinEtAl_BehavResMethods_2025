...
```{r}
path <- "F:/cyepi/code/pre-processing/nonwear_detection/actlumus"
tz <- "Europe/Berlin"
...
```{r}
filepath <- here("F:/cyepi/code/pre-processing/nonwear_detection/wearlog")
...
```{r}
#First, while all participants took part in the experiment for 7 days, they started at different times, and so it is important not just to approximate to 7 days (168 hours), but to calculate the specific 100% of "participation time" for each participant

particip_int <- filtered_time %>%
  mutate(par_int = lubridate::interval(start, end),
         length_par = lubridate::int_length(par_int) %>% as_hms()) %>% #hours of participation for each participant
...
states_dist_jitter <- ggplot(data = int_types, aes(x=per_length, y = State, fill = State)) +
  ...
  labs(title = "(Non-)wear time distribution across the week", x = "Time (%)") +
...
##Visualising weekend and weekdays of non-wear
```{r}
#tot day1
day_one <- filtered_time %>%
  mutate(mon_day = as.Date(start, tz = tz),
         day_one = format(mon_day, "%Y-%m-%d 23:59:59"))
...
off_dist <- joined_int %>%
  ...
  mutate(Date = as.Date(int_end(Interval)), #first, Date and corresponding weekday are calculated
         day = format(Date, format = "%A", locale="English") %>% 
           forcats::fct_inorder()) %>%
  filter(!is.na(State) & State == "off") %>%
  group_by(Id, day, .add = TRUE) %>%
  summarize(int_length = sum(int_length)) %>%
  ungroup()


off_dist <- off_dist %>%
  left_join(day_one, by = "Id")


weekly_nonwear <- off_states %>%
  mutate(day_type = ifelse(day %in% c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday"), "weekday", "weekend_day"))

weekly_nonwear <- weekly_nonwear %>%
  group_by(Id, day_type) %>%
  summarise(non_wear = sum(off_time))

weekly_per_nonwear <- weekly_nonwear %>%
  group_by(Id) %>%
  mutate(per_nonwear = non_wear/if_else(day_type == "weekday", 5, 2)/24 * 100)

dis_daytype <- ggplot(data = weekly_per_nonwear, aes(x = per_nonwear, y= day_type, fill = day_type))  +
 # xlim(NA, 100) +
  ggdist::stat_slab(aes(thickness = after_stat(pdf*n)), scale = 0.5, adjust = .5) +
  ggdist::stat_dotsinterval(side = "bottom", scale = 0.3, slab_colour = NA, slab_linewidth = NA) +
  scale_fill_manual(
    name = "Day type",
    values = c("weekday" = "#4479E4", "weekend_day" = "#9DB9F1"), 
    labels = c("Weekday", "Weekend day"),
    guide = guide_legend(reverse=TRUE)) +
  theme_classic() +
  labs(title = "Non-wear time distribution") +
  xlab("Non-wear time (%)") +
  ggpubr::rremove("ylab") +
  ggpubr::rremove("y.ticks") +
...
ggsave(filename = "nonwear_dist_daytype.png",
       plot = dis_daytype,
       width = 10,
       height = 5,
       dpi = 600,
...
###Ideas and feedback
- Add a beeswarm plot under the desnity plot (instead of a histogram)
- Or add the real data points (geom_jitter)
- Plot for each participant the raw data: light, on/off, button press and activity
