...
summary_plot_all <- ggplot(stacked_overview_long, aes(x = count, y = factor(event, custom_order_stacked), fill = time)) +
...
mostentries_leastentries <- stacked_overview_long %>%
  filter(record_id == "201" | record_id == "213")

mostentries_leastentries_p <- ggplot(mostentries_leastentries, aes(x = count, y = factor(event, custom_order_stacked), fill = time)) +
  geom_bar(position = "stack", stat = "identity")  +
  facet_wrap(~record_id, ncol = 1, scales = "fixed") + 
  scale_y_discrete(breaks = custom_order_stacked, labels = c("Re-entering Tübingen", "Leaving Tübingen", "Off and black bag not used", "Off and black bag used", "Off before sleep ", "Off", "On", "Button press", "All wear log entries")) +
scale_fill_manual(name = "Time of reporting", values = c("#66CCFF", "#0066CC","#006633", "#CC66FF", "#FF99FF", "#FF9933", "#CC0033", "#009933"), labels = c( "Current", "Retrospective", "z1", "z2", "z3","z4", "z5", "z6" )) +
   ggtitle(bquote("Comparison between self-reported \"Wear log\" entries for two participants")) +
  theme(plot.title = element_text(hjust = 0.5, size = 12, face = "bold")) +
  xlab("Count") +
  ylab("Event") +
   theme_bw() +
  scale_x_continuous(breaks = c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60), labels = c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60)) +
        theme(legend.key.size = unit(0.3, "cm")) +
  theme(legend.position = "top",
        legend.direction = "horizontal")  +
   theme(plot.title = element_text(hjust = 0.5, size = 12, face = "bold"))
...
```{r}
ggsave(plot = mostentries_leastentries_p, filename ="most_leastentries.png", dpi =600, width = 9, height = 10, path ="D:/cyepi/code/outputs")
ggsave(plot = mostentries_leastentries_p, filename ="most_leastentries.pdf", dpi =600, width = 12, height = 14, path ="D:/cyepi/code/outputs")
```
