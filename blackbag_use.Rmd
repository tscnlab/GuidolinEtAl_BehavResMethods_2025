...
roc_pim <- pimclusters_clean %>%
 ...
 #Add the result to the list
  roc_pim_list[[as.character(threshold)]] <- roc_pim
}

#Turn the list with the results into a df 
total_roc <- bind_rows(roc_pim_list)
...
activity_medi <- ggplot() +
  geom_point(data = total_roc, aes(x = TPR, y = PPV, color = threshold)) +
 # geom_smooth() +
  xlim(0,1) + 
  ylim(0,1) +
  ...
  geom_point(data = roc_medi, aes(x = TPR, y = PPV, color = medi_threshold)) +
   scale_color_gradient(
  ...
  labs(x="True positive rate (recall)",
       ...
       title = "Precision recall curve for detection \nof non-wear intervals based on illuminance and activity thresholds") +
  theme_bw() +
  coord_fixed(ratio = 1)

ggsave(filename = "activity_medi.png",
       plot = activity_medi,
       height = 6, 
       width = 6,
       dpi = 600,
