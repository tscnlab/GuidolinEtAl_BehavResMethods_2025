...
### Sanity check: dataset where there are no sleep states at all
```{r}
df.LL.onoff <- dataset.LL.wrlg %>%
  filter(! State == "sleep") 
...
#Turn list into df
onoff_prcurve_medi <- bind_rows(prc_list)
#Turn threshold to numeric for plotting
onoff_prcurve_medi$threshold <- as.numeric(onoff_prcurve_medi$threshold)

onoff_prcurve_medi_f1 <- onoff_prcurve_medi %>%
  mutate(f1_score = (2* PPV * TPR)/(PPV + TPR))

```

```{r}
dusk.presleep.LL.wrlg <- photoperioded.dataset.LL.wrlg %>%
  filter(photoperiod.state == "night" & State != "sleep")
...
#Turn list into df
dusk.presleep_prcurve_medi <- bind_rows(prc_list)
#Turn threshold to numeric for plotting
dusk.presleep_prcurve_medi$threshold <- as.numeric(dusk.presleep_prcurve_medi$threshold)

dusk.presleep_prcurve_medi_f1 <- dusk.presleep_prcurve_medi %>%
  mutate(f1_score = (2* PPV * TPR)/(PPV + TPR))
