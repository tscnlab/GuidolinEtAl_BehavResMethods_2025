...
weighted_ppv <- sum(prcurve_medi$PPV * prcurve_medi$n_valid) / sum(prcurve_medi$n_valid)
weighted_tpr <- sum(prcurve_medi$TPR * prcurve_medi$n_valid) / sum(prcurve_medi$n_valid)
weighted_fpr <- sum(prcurve_medi$FPR * prcurve_medi$n_valid) / sum(prcurve_medi$n_valid)
weighted_npv <- sum(prcurve_medi$NPV * prcurve_medi$n_valid) / sum(prcurve_medi$n_valid)

overall_metrics <- tibble(
  weighted_PPV = weighted_ppv,
...
## Comparing algorithm performance during daytime and evening periods
We will use the photoperiod function of LightLogR for this 
```{r}
# Specify experimental coordinates
tubi_coordinates <- c(48.521637, 9.057645)

# Add photoperiod col to the dataset
# Note that we use the dataset where sleep states are NA
photoperioded.dataset.LL.wrlg <- df.LL.nosleep %>%
  LightLogR::add_photoperiod(tubi_coordinates)

# Separate the datasets so that one corresponds to the day and one to the dusk 
daytime.LL.wrlg <- photoperioded.dataset.LL.wrlg %>%
  filter(photoperiod.state == "day")

dusktime.LL.wrlg <- photoperioded.dataset.LL.wrlg %>%
  filter(photoperiod.state == "night")
```

### Measuring algorithm performance for daytime
We perform the same PRC analysis as for the whole dataset. We take the same input parameters of the algorithm as above (min length: 21 minutes, min interruption: 0 minutes) and iterate over mEDI values from 1 to 10 lux.
```{r}
medithresholds <- seq(1, 10, by = 1)
...
#Turn list into df
daytime_prcurve_medi <- bind_rows(prc_list)
#Turn threshold to numeric for plotting
daytime_prcurve_medi$threshold <- as.numeric(daytime_prcurve_medi$threshold)


daytime_prcurve_medi_f1 <- daytime_prcurve_medi %>%
  mutate(f1_score = (2* PPV * TPR)/(PPV + TPR),
         photoperiod = "day")

# Visualisation 
daytime_prc_medi_thresholds <- ggplot() +
   xlim(0,1) + 
   ylim(0,1) +
  geom_abline(slope = 0, intercept = 0.5, linetype = "dashed", color = "darkgrey") + # baseline classifier
  geom_point(data = daytime_prcurve_medi_f1, aes(x = TPR, y = PPV, colour = threshold)) +
  scale_colour_gradient(
     name = "mEDI threshold (lx)",
     low = "red", high = "orange",
     limits= c(1,10),
...
### Measuring algorithm performance for nighttime
We perform the same PRC analysis as for the whole dataset. We take the same input parameters of the algorithm as above (min length: 21 minutes, min interruption: 0 minutes) and iterate over mEDI values from 1 to 10 lux.
```{r}
medithresholds <- seq(1, 10, by = 1)
...
#Turn list into df
dusktime_prcurve_medi <- bind_rows(prc_list)
#Turn threshold to numeric for plotting
dusktime_prcurve_medi$threshold <- as.numeric(prcurve_medi$threshold)


dusktime_prcurve_medi_f1 <- dusktime_prcurve_medi %>%
  mutate(f1_score = (2* PPV * TPR)/(PPV + TPR),
         photoperiod = "night")

# Visualisation 
dusktime_prc_medi_thresholds <- ggplot() +
   xlim(0,1) + 
   ylim(0,1) +
  geom_abline(slope = 0, intercept = 0.5, linetype = "dashed", color = "darkgrey") + # baseline classifier
  geom_point(data = dusktime_prcurve_medi_f1, aes(x = TPR, y = PPV, colour = threshold)) +
  scale_colour_gradient(
     name = "mEDI threshold (lx)",
     low = "red", high = "orange",
     limits= c(1,10),
...
## Weighing algorithm performance by number of labelled observations
```{r}
# Combine day and dusk results
all_prc <- bind_rows(daytime_prcurve_medi_f1, dusktime_prcurve_medi_f1)

ggplot(all_prc, aes(x = threshold, y = f1_score, color = photoperiod)) +
  geom_line() +
  geom_point() +  # size reflects sample size
  labs(title = "Algorithm F1 Performance by Photoperiod",
       y = "F1 Score", x = "MEDI Threshold") +
  theme_minimal()

# Weighted across groups per threshold
weighted_results <- all_prc %>%
  group_by(threshold) %>%
  summarise(
    weighted_TPR = sum(TPR * n_valid) / sum(n_valid),
    weighted_PPV = sum(PPV * n_valid) / sum(n_valid),
    total_n_valid = sum(n_valid)
  ) %>%
  mutate(weighted_F1 = (2 * weighted_TPR * weighted_PPV) / 
                       (weighted_TPR + weighted_PPV))

```

### Re-create PRC visualisation of combined daytime and dusk time, without activity
```{r}
combined_prc_medi_thresholds <- ggplot() +
   xlim(0,1) + 
   ylim(0,1) +
  geom_abline(slope = 0, intercept = 0.5, linetype = "dashed", color = "darkgrey") + # baseline classifier
  geom_point(data = prcurve_medi_f1, aes(x = TPR, y = PPV, colour = threshold)) +
  scale_colour_gradient(
     name = "mEDI threshold (lx)",
     low = "red", high = "orange",
     limits= c(1,10),
...
multiplot <- cowplot::plot_grid(combined_prc_medi_thresholds, daytime_prc_medi_thresholds, dusktime_prc_medi_thresholds,
                                labels = c("A", "B", "C"),
                                ncol = 3,
                                nrow = 1,
                                align = "hv")

ggsave(filename = "daytime_dusk_multiplot.png",
       plot = multiplot,
       width = 14, 
       height = 6,
       dpi = 600, 
