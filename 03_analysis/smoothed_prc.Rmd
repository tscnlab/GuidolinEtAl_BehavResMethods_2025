...
```{r}
#Before we transform into log, we replace 0 values with low numbers
dataset.LL.wrlg$MEDI[dataset.LL.wrlg$MEDI == 0] <- 0.0001
dataset.LL.wrlg$PIM[dataset.LL.wrlg$PIM == 0] <- 0.0001

#Take log10 of both MEDI and PIM
log.LL.wrlg <- dataset.LL.wrlg %>%
...
For now, we do it without this step
...
prc_pim_list <- list()
  ...
  pimclusters_clean <- pim_clusters %>%
   select(Datetime, State, bag, is_low_pim_cluster) %>%
    mutate(
      State = case_when(
      State == "on" ~ 1, 
      State == "off" ~ 0),
...
prcurve_pim.smoothed <- bind_rows(prc_pim_list)

#Turn thresholds from character to factor for plotting
prcurve_pim.smoothed$threshold <- as.numeric(prcurve_pim.smoothed$threshold)
```
...
#Turn the list with the results into a df 
prcurve_medi.smoothed <- bind_rows(prc_medi_list)

#Turn thresholds from character to factor for plotting
prcurve_medi.smoothed$threshold <- as.numeric(prcurve_medi.smoothed$threshold)
```
...
prc_smoothed <- ggplot() +
  ...
  #First plot: activity
  geom_point(data = prcurve_pim.smoothed, aes(x = TPR, y = PPV, color = threshold)) +
  ...
  #Second plot: light
  geom_point(data = prcurve_medi.smoothed, aes(x = TPR, y = PPV, colour = threshold)) +
...
ggsave(filename = "prc_smoothed_grouped.png",
       ...
       dpi = 600,
       bg = "white",
       path= "G:/cyepi/code/outputs/light_activity_prc")
```
...
#Save the plot
ggsave(filename = "prc-multiplot-raw-unlabelled.png",
       plot = prc_multiplot,
       height = 10, 
       width = 10,
       dpi = 600,
