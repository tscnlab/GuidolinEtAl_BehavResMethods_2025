...
## Save plots above
```{r}
ggsave(filename = "states-distribution-1axis.png",
       plot = states_dist_jitter2 ,
       width =8,
       height = 5,
       dpi = 600,
       path= "G:/cyepi/code/outputs/wrlg_plots")
...
# Saving the plot
ggsave(filename = "results_fig02b.pdf",
...
wknd_wkday <- wknd_wkday.combined %>%
 ...
 wknd_wk_dist <- ggplot(data = wknd_wkday, aes(x=perc_length, y = day_type, fill = day_type)) +
  ...
  ggpubr::rremove("y.ticks") +
 # ggpubr::rremove("xlab")+ 
  labs(title = "Non-wear time distribution across \nweekdays and weekend days", x = "Percentage of time (%)") +
...
```{r}
 #Saving the plot
 ggsave(filename = "results_fig02c.svg",
       plot =  wknd_wk_dist,
       width = 7,
       height = 4,
       dpi = 600,
...
fig02_multiplot <- state_dist_2axis_2/wknd_wk_dist/timing_dens_plot 
