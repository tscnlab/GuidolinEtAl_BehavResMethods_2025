...
Input values: 10 mins, 1 min interruption, 2 lux 
```{r}
...
medi_sc <- mediclusters_clean %>%
  ...
  ungroup() %>%
  select(-c(interval_group, end)) %>%
  rename(Datetime = start) #need to transform start to Datetime for next step (sc2interval)

#Now, we want to convert the state changes into intervals. We need this to be able to plot the classification output.
medi_int <- medi_sc %>%
  group_by(Id) %>%
  sc2interval(Statechange.colname = classification) # We use sc2interval to turn the changes (in this case changes in algorithm output) into an interval 
...
alg.combined$State <- factor(alg.combined$State, levels = c("TP", "TN", "FP", "FN")) #turn State into factor
...
p <- gg_day(data = subset_df, #base plot defaults to plotting mEDI on y axis
  ...
  geom_rect(data=alg.combined, aes(xmin= xmin, xmax = xmax, ymin =0, ymax = Inf, fill = State), #adding classification output
                    inherit.aes = FALSE,
                    alpha = 0.15) +
    scale_fill_manual(
