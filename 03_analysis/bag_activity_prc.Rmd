...
#Finding gaps of time (which correspond to sleep)
mediclusters_gaps <- mediclusters_clean %>%
  gap_handler(Datetime.colname = Datetime,
              behavior = "gaps")
...
medi_sc <- mediclusters_clean %>%
 # group_by(Id) %>%
  mutate(change = classification != lag(classification, #stores whether classification has changed in a "change" column
