...
x_box <- ggplot(median.LL.wrlg, aes(x = median_medi, fill = State, color = State)) +
...
y_box <- ggplot(median.LL.wrlg, aes(x = median_pim, fill = State, colour = State)) +
  geom_boxplot(alpha = 0.6) +
  scale_fill_manual(
    values = c("off" = "darkred", "on" = "#0072B2", "sleep" = "darkgoldenrod2")
  ) +
  ...
  coord_flip() +  # Flip for alignment along the y-axis
  theme_void() +
  theme(legend.position = "none")
...
combined_plot <- cowplot::plot_grid(
  scatter_with_marginals_grob, # First plot with marginal
  ggplotGrob(prc_combined),    # Second plot
  ncol = 2, align = "h", axis = "tb" # Align horizontally
)
...
medi_pim_fig <- cowplot::plot_grid(plot_marginal,
                          ...
                          rel_heights = c(1.5,0.9))


scatter_xbox <- (
  x_box / scatter + # x_box above scatter
    plot_layout(heights = c(0.7, 5),
                widths = c(1,5))
  ) | (y_box)  # Ensure y_box aligns with scatter


p <- x_box + patchwork::plot_spacer() + scatter + y_box + 
  plot_layout(widths = c(4,2), heights = c(1,5))

  patchwork::plot_annotation(tag_levels = list(c("A", "B"))) +
  patchwork::plot_layout(widths = c(1,0.5))
# Save the plot
...
ggsave(filename = "results_fig05.png",
       ...
       dpi = 600,
       bg = "white",
       path= "H:/nonwear_detection/preprint_figures/results/fig05")
