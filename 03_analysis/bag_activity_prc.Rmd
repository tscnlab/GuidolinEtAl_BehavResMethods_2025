...
for (threshold in medithresholds) {
  ...
  #First, we need to convert all sleep values to NAs, since we do not want to use these for our classification 
medi_clusters <- dataset.LL.wrlg %>%
  filter(Id == "208") %>% #eliminate, this is just trying out with only 1 participant
  ungroup() %>%
  mutate(State = if_else(State == "sleep", "NA", State)) %>% 
  # Replace `low_activity` by the (logical) variable for which the clusters 
  # should be found.
...
## Visualising the low medi clusters to ensure the min length is indeed X minutes
```{r}
cluster_dur_2 <- medi_clusters %>%
  ...
  filter(is_low_medi_cluster == TRUE) 

#Checking whether there are clusters that are less than 10 mins 
check_dur <- cluster_dur_2 %>%
  filter(cluster_length < "2 mins")

#Visualising this in an histogram
