...
prc_medi <- mediclusters_clean %>%
  group_by(Id, classification) %>% #!! if you want to visualise individual PRC, then you have to group_by(Id, classification). Else, only group_by(classification)
  summarise(count = n()) %>%
  pivot_wider(names_from = classification, values_from = count, values_fill = list(count=0)) %>%
...
# The point about combining light and activity
Combining light and activity would help us to identify cases where:
- Activity is low (e.g. participant sitting), but light exposure is high -> as wear time



#New visualisation: plotting the classification outcome as a timeseries
We will do this initially for just one participant
```{r}
#Define PIM clusters
