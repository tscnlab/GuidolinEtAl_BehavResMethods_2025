...
#[This might be eliminated] Before identifying clusters of low activity (PIM), we need to understand: what is low activity?
```{r}
pim_stats <- dataset.LL.wrlg %>%
  filter(!State == "sleep") %>%
  group_by(Id) %>%
  summarise(mean = mean(PIM),
            median = median(PIM), 
            min = min(PIM),
            max = max(PIM))
```
##Tranforming the data so that we have 
...
### SOLUTION: just join medicluster_clean with the original dataset.LL.wrlg? 

# We use sc2interveal to turn the changes (in this case changes in algorithm output) into an interval 

filepath = "G:/cyepi/code/pre-processing/nonwear_detection/alg_states.csv"

write.csv(mediclusters_clean, "G:/cyepi/code/pre-processing/nonwear_detection/alg_states.csv")

mediclsuters_import = list.files(filepath, pattern="*.csv")

alg_states <- import_Statechanges(mediclsuters_import,
                                    path = filepath,
                                    State.colnames = c("TN", "TP", "FN", "FP"),
                                    State.encoding = c("TN", "TP", "FN", "FP"))
  ...
  select(-State) %>%
  group_by(Id) %>%
  sc2interval(Datetime.colname = Datetime,
              Statechange.colname = classification,
              Datetime.keep = TRUE
              ) %>%
  ungroup()

algor_int_pim <- pimclusters_clean %>%
  group_by(Id) %>%
  

algor_LL_wrlg <- clean.LL.wrlg %>%
  interval2state(algor_int, State.colname = classification)


sleepstate <- algor_LL_wrlg %>%
   filter(State == "sleep")
...
state.times %>% filter(Day.data.min != Day.data.max) %>%
  ...
  select(Id, State, classification, new_entry) %>%
  unnest(new_entry)
...
ggsave(filename = filename,
       ...
       dpi = 600,
       bg = "white",
       path= "G:/cyepi/code/outputs/visual_insp/light")
}
```
