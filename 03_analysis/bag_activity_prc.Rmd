...
#4) State = 1, cluster = 0 -> true negative
  ...
  #Add the result to the list
   prc_pim_list[[threshold]] <- prc_pim
  }

#Turn the list with the results into a df 
prcurve_pim <- bind_rows(prc_pim_list)

#Turn thresholds from character to numeric for plotting
...
ggsave(filename = "10min-index-prc-raw.png",
       ...
       dpi = 600,
       bg = "white",
       path= "D:/cyepi/code/outputs/light_activity_prc/raw/trblshoot_parameters")
...
#Empty list to store classification results
prc_medi_list <- list()
min_length = 60 #1 minutes of observation, with sampling interval of 10 seconds -> 60 rows
...
prc_medi <- mediclusters_clean %>%
 ...
 #Add the result to the list
  prc_medi_list[[threshold]] <- prc_medi
}
