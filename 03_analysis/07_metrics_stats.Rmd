...
posthoc_equiv_table <- equiv_results %>%
  ...
  gt::gt() %>% 
  gt::tab_header(title = gt::md("**Post-hoc pairwise comparisons and equivalence test results**"),
...
## Table for meaningfully positive or negative effects of the equivalence test
```{r}
equiv_meanginful_results <- equiv_results %>%
  ...
  # Superscript formatting using HTML
  # mutate(
  #   metric = case_when(
  #   metric == "tat1000" ~ gt::html("TAT<sup>1000</sup>"),
  #   metric == "tat250" ~ gt::html("TAT<sup>250</sup>"),
  #   metric == "mean" ~ gt::html("Average daytime mEDI"),
  #   metric == "m10" ~ gt::html("M10m")
  #   )
  # ) %>%
  ...
  gt::text_transform(
    ...
    fn = function(val) {
      if (starts_with("TAT")) {
      sup <- stringr::str_extract(val, "\\d+")
      glue::glue("TAT<sup>{sup}</sup>")
    } else 
  ) %>%
  gt::cols_label(metric = "Metric",
                 equivalence = "Equivalence test result",
