...
run_bootstrap_equiv_test <- function(metric_name, group1, group2, light_metrics_list,
  ...
  prop_inside_SESOI <- mean(bootstraps >= -SESOI & bootstraps <= SESOI, na.rm = TRUE)
  prop_outside_SESOI <- mean(bootstraps < -SESOI | bootstraps > SESOI, na.rm = TRUE)
...
- If both ends of the CI lie within SESOI bounds, then your data is statistically equivalent at this alpha level — no meaningful difference.
- If any part of the CI extends outside SESOI, can’t conclude equivalence.
...
noplot_bootstrap_equiv_test <- function(metric_name, group1, group2, light_metrics_list,
                                     n_bootstraps = 10^4, SESOI = NULL, alpha = 0.05) {
  ...
  # Add 'mean_' prefix if needed (is and iv)
  special_metrics <- c("iv_all", "is_all")
  if (!(metric_name %in% special_metrics)) {
    if (!(group1 %in% names(df))) group1 <- paste0("mean_", group1)
    if (!(group2 %in% names(df))) group2 <- paste0("mean_", group2)
  }
  ...
  # Proportions inside/outside SESOI
  prop_inside_SESOI <- mean(bootstraps >= -SESOI & bootstraps <= SESOI, na.rm = TRUE)
  prop_outside_SESOI <- mean(bootstraps < -SESOI | bootstraps > SESOI, na.rm = TRUE)
...
for (i in seq_len(nrow(posthoc_all))) {
  ...
  for (s in sesoi_seq) {
    ...
    res <- noplot_bootstrap_equiv_test(
      ...
      group1 = group1,
      group2 = group2,
      light_metrics_list = light_metrics_list,
      SESOI = s
    )
