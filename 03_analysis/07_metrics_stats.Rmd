...
### Performing the non-parametric Friedman test for the metrics
#### First, we  create a function to perform the test for all metrics
```{r}
# Friedman test
...
friedman_test_perform <- function (df) {
...
# Turning df into long form to perform computations
  ...
  friedmantest <- df_long %>%
    rstatix::friedman_test(mean ~ dataset | Id) 
  
  # Adding the effect size (Kendall's W)
  kendalls_w <- df_long %>%
    rstatix::friedman_effsize(mean ~ dataset | Id)
  
  # Merging into only one df
  friedmantest <- friedmantest %>%
    mutate(kendallsw = kendalls_w$effsize) # added as new column
  
  # Adding a column indicating which metric the test corresponds to
  friedmantest$metric <- (df_name)
...
#MLiT250
mlit250_friedman <- friedman_test_perform(mlit250_all)
...
# Daily average
mean_friedman <- friedman_test_perform(mean_all)
