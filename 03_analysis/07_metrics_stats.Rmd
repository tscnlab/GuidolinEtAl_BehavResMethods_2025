...
```{r}
run_bootstrap_equiv_test <- function(metric_name, group1, group2, metrics_list,
                                     n_bootstraps = 10^4, SESOI = 0.2, alpha = 0.05) {
  
  # Pull the original data
  df <- metrics_list[[metric_name]]
  ...
  # Checking if cols are correctl identified 
...
metric_name <- "tat1000_all"
group1 <- "mean_clusters"
group2 <- "mean_wrlg"

# Run the equivalence test for this one case
result_single <- run_bootstrap_equiv_test(metric_name, group1, group2, metrics_list)
```

## Try out on all pairwise combinations from the post-hoc tests

```{r}
# Example: join equivalence to your post-hoc results
equiv_results <- posthoc_all %>%
  mutate(equiv_result = pmap(
    ...
    ~ run_bootstrap_equiv_test(..1, ..2, ..3, metrics_list)
  )) %>%
