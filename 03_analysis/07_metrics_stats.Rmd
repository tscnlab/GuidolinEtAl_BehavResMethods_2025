  ...
  gt::text_transform(
    ...
    sapply(x, function(val) {
      if (str_detect(val, "^tat\\d+")) {
        ...
        sup <- str_extract(val, "\\d+")
        gt::html(glue::glue("LLiT<sup>{sup}</sup>"))
      } else {
        gt::html(val)
      }
    })
...
## Third: Table for meaningfully positive or negative effects of the equivalence test
```{r}
...
- TAT1000 and TAT250: raw wearlog values are higher in raw dataset compared to datasets cleaned by either wear log +
