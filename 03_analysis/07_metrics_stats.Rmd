...
### Apply function to all metrics dfs
```{r}
library(purrr)
...
results <- lapply(names(metrics_list), function(nm) {
  fried_res <- friedman_and_posthoc(metrics_list[[nm]], nm, 0.05)
  
  if (fried_res$friedman$p >= 0.05) {
    equiv_res <- perform_equivalence_tests(metrics_list[[nm]], nm, 0.05, 0.5)
  } else {
    equiv_res <- NULL
  }
  
  list(friedman = fried_res, equivalence = equiv_res)
})

# Save results in dfs
friedman_all <- purrr::map_dfr(results_list, "friedman")
posthoc_all  <- purrr::map_dfr(results_list, "posthoc") 
```
