...
# We still need to add a legend to all of the plots - one for the Id and one for the histogram of the deltas. We can extract this from a plot
## First, extra the Id legend by creating a scatterplot for this purpose
clean_wrlg <- ggpubr::ggscatter(data = is_all,
          x = "IS_raw",
          y = "IS_wrlg",
          color = "Id") +
  guides(color = guide_legend(nrow = 4)) + # Set the number of rows
  theme(legend.position = "top",
        legend.title = element_text((size =14))) 

#Extract legend
legend_ids <- ggpubr::get_legend(clean_wrlg)
# Turn it into a ggplot item
legend_ids_ggplot <- ggpubr::as_ggplot(legend_ids) 

## Second, extract the legend for the histogram of the deltas
delta_hist <- ggplot(is_all_long, aes(x = delta, fill = metric)) +
  geom_histogram() +
  labs(x = "Delta", y = "Frequency") +
  scale_fill_manual(
...
#Extract legend
legend_metric <- ggpubr::get_legend(delta_hist)
# Turn it into a ggplot item
legend_metrics_ggplot <- ggpubr::as_ggplot(legend_metric) 


# Combine the two legends into a single row
combined_legends <- cowplot::plot_grid(
  legend_ids_ggplot, NULL, legend_metrics_ggplot,
  ncol = 3, # Arrange in a single row
  nrow = 1,
  rel_widths = c(1.7, 0.1, 1) # Adjust relative widths if needed
)
...
## Adding the legend on top of the first plot
final_plot_1 <- plot_grid(
  combined_legends, plots1,
  ncol = 1, 
  nrow = 2, 
  rel_heights = c(0.5,6.5) # Adjust the relative heights iso legend is small
)

## Adding the legend on top of the second plot
final_plot_2 <- plot_grid(
  combined_legends, plots2,
  ncol = 1, 
  nrow = 2, 
  rel_heights = c(0.5,6.5) # Adjust the relative heights iso legend is small
)
...
```{r}
ggsave(filename = "results_fig08_1.png",
       plot = final_plot_1, 
       width = 8,
       height = 15.5,
       dpi = 600,
