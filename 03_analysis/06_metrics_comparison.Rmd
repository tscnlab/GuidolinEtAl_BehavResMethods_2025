...
clean_wrlg <- ggpubr::ggscatter(data = is_all,
  ...
  labs(x = "Raw dataset", y = "Wear log-corrected dataset") +
  coord_fixed(ratio = 1) +
...
clean_clusters <- ggpubr::ggscatter(data = is_all,
  ...
  labs(x = "Raw dataset", y = "Algorithm-corrected dataset") +
  coord_fixed(ratio = 1) +
  ggpubr::rremove("legend") +
  theme(plot.margin = unit(c(t=0,r=0.1,b=0.2,l=0), "cm"))
...
raw_vs_wrlg_vis <- visualize_comparison(
  ...
  y_col = "mean_wrlg", 
  x_label = "Raw dataset", 
  y_label = "Wear log-corrected dataset",
)
...
raw_vs_clusters_vis <- visualize_comparison(
  ...
  y_col = "mean_clusters", 
  x_label = "Raw dataset", 
  y_label = "Algorithm-corrected dataset",
)
...
m10_raw <- m10_raw_wday %>%
  ...
  summarize(mean_raw = mean(brightest_10h_mean, na.rm = TRUE),
            .groups = "drop")
...
m10_wrlg <- m10_wrlg_wday %>%
  ...
  mutate(brightest_10h_mean = exp_zero_inflated(brightest_10h_mean)) %>%
  summarize(
    mean_wrlg = mean(brightest_10h_mean, na.rm = TRUE),
    .groups = "drop"
  )
...
m10_clusters <- m10_clusters_wday %>%
  ...
  mutate(brightest_10h_mean = exp_zero_inflated(brightest_10h_mean)) %>%
  summarize(
    mean_clusters = mean(brightest_10h_mean, na.rm = TRUE),
    .groups = "drop"
  )
...
#### Saving the metrics as csv files (optional)
```{r}
...
# Save each metric data frame to a separate CSV
for (name in names(light_metrics_list)) {
  write_csv(
    light_metrics_list[[name]], 
