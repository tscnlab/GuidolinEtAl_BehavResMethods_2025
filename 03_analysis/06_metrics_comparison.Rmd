...
- TAT250, TAT1000
- 
- M10 (mean of the brightest cotinuous period)
```{r}
...
batch_raw <- 
  ...
  summarize(
    ...
    average_MEDI = 
       exp_zero_inflated(mean(log_zero_inflated(MEDI), na.rm = TRUE)), #log transforming in base ten after adding a small number, then back transforming
    median_MEDI = 
      median(MEDI, na.rm = TRUE),
    M10 =
      bright_dark_period(MEDI, Datetime, na.rm = TRUE, as.df = TRUE),
    .groups = "drop_last"
    ) %>% 
  unnest(-Id)
...
batch_wrlg <- 
  ...
  summarize(
    ...
    TAT1000 = 
      duration_above_threshold(MEDI, Time, threshold = 1000, as.df = TRUE, na.rm = TRUE),
    average_MEDI = 
       exp_zero_inflated(mean(log_zero_inflated(MEDI), na.rm = TRUE)),
    median_MEDI = 
      median(MEDI, na.rm = TRUE),
    M10 =
      bright_dark_period(MEDI, Datetime, na.rm = TRUE, as.df = TRUE),
...
```
### Mean comparison 
We calculate the average daily MEDI (for this, we use the log zero inflated approach proposed [here](https://tscnlab.github.io/LightLogR/articles/log.html)). The day iinformation is based on the given photoperiod of provided coordinates.
```{r}
#specifying coordinates (latitude/longitude)
coordinates <- c(48.521637, 9.057645)
...
```{r}
# Raw dataset
m10_raw <- raw_dataset_wdays %>%
  group_by(Id) %>%
  mutate(m10 = bright_dark_period(exp_zero_inflated(mean(log_zero_inflated(MEDI),
                                                         Datetime, as.df = TRUE))))
                                              
# Wear log dataset
m10_wrlg <- batch_wrlg %>%
  group_by(Id) %>%
  summarise(m10_wrlg = mean(brightest_10h_mean))

# Algorithm dataset
m10_clusters <- batch_clusters %>%
  group_by(Id) %>%
  summarise(m10_clusters=mean(brightest_10h_mean))
...
clean_wrlg <- ggpubr::ggscatter(data = m10_all,
  ...
  ggplot2::scale_x_continuous(
    limits = c(50, 3700),
    breaks = c(50, 1875, 3700), 
    labels = c("50", "1875", "3700")
  ) +
  scale_y_continuous(
    limits = c(50, 3700),
    breaks = c(50, 1875, 3700), 
