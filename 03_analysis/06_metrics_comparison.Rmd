...
m10_raw <- raw_dataset_wdays %>%
  group_by(Id, wDay) %>%
  summarize(
    brightest_10h_mean = bright_dark_period(MEDI, Datetime, as.df = TRUE)$brightest_10h_mean,
    .groups = "drop"
  ) %>%
  group_by(Id) %>%
  summarize(
    m10_raw = mean(brightest_10h_mean, na.rm = TRUE),
    .groups = "drop"
  )

# Wear log dataset
m10_wrlg <- nw_wrlg_wdays %>%
  group_by(Id, wDay) %>%
  summarize(
    brightest_10h_mean = bright_dark_period(MEDI, Datetime, as.df = TRUE)$brightest_10h_mean,
    .groups = "drop"
...
# Algorithm dataset
m10_clusters <- nw_clusters_wdays %>%
  group_by(Id, wDay) %>%
  summarize(
    brightest_10h_mean = bright_dark_period(MEDI, Datetime, as.df = TRUE)$brightest_10h_mean,
    .groups = "drop"
...
clean_wrlg <- ggpubr::ggscatter(data = m10_all,
  ...
  ggplot2::scale_x_continuous(
    limits = c(50, 3285),
    breaks = c(50, 1667, 3285), 
    labels = c("50", "1667", "3285")
  ) +
  scale_y_continuous(
    limits = c(50, 3285),
    breaks = c(50, 1667, 3285), 
...
delta_hist <- ggplot(m10_all_long, aes(x = delta, fill = metric)) +
  ...
  scale_x_continuous(
    limits= c(-1040, 1040),
    breaks = c(-1040, 0.00, 1040),
    labels = c("-1040", "0", "1040")) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5), 
        aspect.ratio = 1,
        plot.margin = unit(c(t=0,r=0.1,b=0.2,l=0), "cm")) +
