...
-  FLIT10, LLIT10
- TAT250, TAT1000
- Average MEDI (for this, we use the log zero inflated approach proposed [here](https://tscnlab.github.io/LightLogR/articles/log.html))
...
```
### Mean comparison 
We have already calculated the daily averages in the batch calculations. Now we want to calculate the average for each participant.
```{r}
#specifying coordinates (latitude/longitude)
coordinates <- c(48.521637, 9.057645)
...
mean_raw2 <- raw_dataset_wdays %>%
  add_photoperiod(coordinates) %>%
  group_by(Id, photoperiod.state, .add=TRUE) %>%
  summarise(mean_raw = mean((MEDI), na.rm = TRUE))

exp_zero_inflated(mean(log_zero_inflated(MEDI), na.rm = TRUE))
# Wear log dataset
mean_wrlg <- batch_wrlg %>%
  group_by(Id) %>%
  summarise(mean_wrlg = mean(average_MEDI))

# Algorithm dataset
mean_clusters <- batch_clusters %>%
  group_by(Id) %>%
  summarise(mean_clusters=mean(average_MEDI))
...
p14 <- clean_wrlg + clean_clusters + delta_hist + 
  patchwork::plot_annotation("Average mEDI",
...
m10_raw <- batch_raw %>%
  group_by(Id) %>%
  summarise(m10_raw = mean(brightest_10h_mean))
