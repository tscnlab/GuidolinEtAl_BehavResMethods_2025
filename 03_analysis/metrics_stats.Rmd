...
ttest_table <- ttest_final %>%
  select(-c(.y., cohens_d)) %>% #we do not need these col in the final summary
  gt::gt() %>%
  ...
  gt::tab_options(table.width=gt::pct(70)) #increasing table width so that it does not get croppes when saving
