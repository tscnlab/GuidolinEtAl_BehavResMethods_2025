...
int_duration <- int_duration %>%
  ...
  mutate(id_duration_n= as.numeric(tot_intlength)/as.numeric(id_duration), #convert to numeric as division between difftime objects is not supported, n stands for "normalised"
...
ggsave(filename = "results_fig05a.png",
       plot = state_dist_2axis_2,
       width = 7,
       height = 4,
       dpi = 600,
       path= here::here("outputs", "fig05"))
```

## 2a) Calculate descriptive statistics on how much time participants spent in each Wear log state across the week 
```{r}
...
## 2b) Calculate summary statistics on length of the non-wear episodes (without aggregating them for each id)
```{r}
# Calculate duration as hms for each off interval
wrlg_int_off <- wrlg_int %>%
  filter(State == "off") %>%
  mutate(off_length = lubridate::int_length(Interval) %>% as_hms())

# Calculate summary statistics
wrlg_int_off_quantiles <- quantile(wrlg_int_off$off_length)
print(wrlg_int_off_quantiles) # returns 0%, 25%, 50% (i.e. median), 75% and 100% quantiles
...
# Saving the plot
ggsave(filename = "results_fig05c.png",
...
```{r}
 #Saving the plot
 ggsave(filename = "results_fig05b.png",
       plot =  wknd_wk_dist,
       width = 7,
       height = 4,
       dpi = 600,
...
ggsave(filename = "results_fig05_all.png",
...
### Calculating the % of non-wear time only during wake periods 
Given that participants were not instructed to wear the sensor during the night. 
```{r}
int_duration_wake <- int_duration %>%
  # Only select on and off periods
  dplyr::filter(!State == "sleep") %>%
  # Select cols of interest
  dplyr::select(-c(id_duration_n, id_duration_per, tot_intlength_hours, id_duration)) %>%
  group_by(Id) %>%
  # Calculate total wake period as sum of off and on periods
  mutate(wakeperiod = as_hms(sum(tot_intlength)
                             ),
         intlength_off = as_hms(sum(tot_intlength[State == "off"])
                                ),
         non_wear_percent = (as.numeric(intlength_off) / as.numeric(wakeperiod))
         *100)

# Summary statistics
int_duration_wake_summary <- int_duration_wake %>%
  ungroup() %>%
  summarise(mean = mean(non_wear_percent),
            sd = sd(non_wear_percent))

print(int_duration_wake_summary)
```
